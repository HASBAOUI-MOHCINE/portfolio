...
const Certifications = () => {
  ...
  return (
    ...
    >
      ...
      <div className="grid sm:grid-cols-2 gap-4 sm:gap-6 mb-16 sm:mb-24">
        {education.map((edu, i) => (
          <div
            key={edu.name}
            className={`group flex items-start gap-3 sm:gap-4 p-4 sm:p-6 rounded-xl sm:rounded-2xl transition-all duration-300 ${
              isDark 
          ...
          >
            <div className={`flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-lg sm:rounded-xl group-hover:text-cyan-500 transition-colors ${
              isDark ? 'bg-gray-900/50 text-gray-400' : 'bg-gray-100 text-gray-500'
            }`}>
              <div className="text-xl sm:text-2xl">{edu.icon}</div>
            </div>
            <div className="min-w-0">
              <h3 className={`text-base sm:text-lg font-semibold mb-1 leading-tight ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{edu.name}</h3>
              <p className={`text-xs sm:text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>{edu.institution}</p>
              <p className={`text-[10px] sm:text-xs mt-1 sm:mt-2 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>{edu.date}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {certifications.map((cert, i) => (
          <div
            key={cert.name}
            className={`group flex flex-col items-center text-center p-5 sm:p-8 rounded-xl sm:rounded-2xl transition-all duration-300 ${
              isDark 
                ? 'bg-gray-800/30 border border-gray-700/50 hover:border-gray-600/50' 
                : 'bg-white/60 border border-gray-200 hover:border-gray-300 shadow-sm'
            }`}
          ...
          >
            <div className={`mb-3 sm:mb-5 w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center rounded-xl sm:rounded-2xl group-hover:text-cyan-500 transition-colors ${
              isDark ? 'bg-gray-900/50 text-gray-400' : 'bg-gray-100 text-gray-500'
            }`}>
              <div className="text-lg sm:text-2xl">{cert.icon}</div>
            </div>
            <h3 className={`text-base sm:text-lg font-semibold mb-1 sm:mb-2 leading-tight ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{cert.name}</h3>
            <p className="text-xs sm:text-sm text-cyan-500 font-medium">{cert.issuer}</p>
            <p className={`text-[10px] sm:text-xs mt-2 sm:mt-3 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>{cert.date}</p>
          </div>
        ))}
      </div>
